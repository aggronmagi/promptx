...
func loginCommand() *promptx.Cmd {
	return promptx.NewCommand("login", "登录游戏",
		...
		ctx.Printf("登录成功: %s 服务器: %s\n", args.Account, args.Server)
	})
}
...
命令执行函数内 使用 ~CommandContext~ 提供的 ~Check...~ 方法或 ~Bind~ 方法获取参数.

Check 方法调用推荐和创建命令时候输入的 ~CommandParameter~ 一一匹配. Index 参数从 0 开始.
